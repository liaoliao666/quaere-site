...
```ts
const anQuery = query({
  // 如果设置了 id，dehrydrate 函数将会保留该 query。
  id: string,
  // 必选，用于请求数据的函数。
...
- `refetch()`: 重新查询
- `isLoading`: 当查询的第一次获取请求正在进行中时，则为 `true`。
- `isFetching`: 如果正在加载请求或重新请求数据
- `isPlaceholderData`: 如果显示的数据是 `placeholderData` 选项返回的占位数据，则为 `true`。

## 选项 [#options]

- `enabled: boolean | (data: TData, queryInfo: QueryInfo) => number`
  - 默认为 `true`
...
- `refetchInterval: number | false | ((data: TData | undefined, queryInfo: QueryInfo) => number | false | undefined)`
  - 如果设置为数字，所有查询将以毫秒为单位以此频率连续重新获取。
  - 如果设置为函数，则将使用最新数据和查询执行函数以计算频率。
- `refetchIntervalInBackground: boolean`
  - 如果设置为 `true`，则设置为连续重新获取的查询将在其选项卡/窗口在后台时继续重新获取。
- `refetchOnMount: boolean | "always" | ((queryInfo: QueryInfo) => boolean | "always")`
  - 默认为 `true`
  - 如果设置为 `true`，如果数据过期，查询将在挂载时重新获取。
...
- `refetchOnWindowFocus: boolean | "always" | ((queryInfo: QueryInfo) => boolean | "always")`
  - 默认为 `true`
  - 如果设置为 `true`，如果数据过期，查询将在窗口获得焦点时重新获取。
  - 如果设置为 `false`，查询将不会在窗口获得焦点时重新获取。
  - 如果设置为 `"always"`，查询将始终在窗口获得焦点时重新获取。
  - 如果设置为函数，则将执行函数以计算值。
